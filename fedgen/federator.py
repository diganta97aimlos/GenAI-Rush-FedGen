...
class SimpleFederatedAveraging():
    ...
    def loadModels(self, trainDataLoader=None, worker=None):
        ...
        encryptedModel.load_state_dict(torch.load(f'/home/ubuntu/kreedaAI/fedgen/models/encrypted_{worker}.pth'))
        return encryptedModel

    def federateGlobalModel(self, trainDataLoader=None):
        logging.info(f'Federating {len(self.workers)} Client Models')
        clientModelsWeights = list()
        for worker in self.workers:
            clientModelsWeights.append(self.loadModels(trainDataLoader=trainDataLoader, worker=worker).state_dict())
        self.federatedModel = self.loadModels(trainDataLoader=trainDataLoader, worker='worker1')
        ...
        torch.save(self.federatedModel.state_dict(), '/home/ubuntu/kreedaAI/fedgen/models/encrypted_global_federated.pth')

    def federateClientModels(self):
        clientModelsWeights = list()
        for worker in self.workers:
            clientModelsWeights.append(torch.load(f'/home/ubuntu/kreedaAI/fedgen/models/encrypted_{worker}.pth'))
        federateGlobalModelWeights = torch.load('/home/ubuntu/kreedaAI/fedgen/models/encrypted_global_federated.pth')
        for idx in range(len(clientModelsWeights)):
            ...
            torch.save(clientModelsWeights[idx], f'/home/ubuntu/kreedaAI/fedgen/models/encrypted_federated_{worker}.pth')